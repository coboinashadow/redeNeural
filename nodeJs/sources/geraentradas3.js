...
for(i=0;i<qtdimagens;i++){
	...
	if(i<qtdimagens/2){//metade das imagens contem circulo
		cx=20+Math.floor(Math.random()*20); //centro do circulo
		cy=20+Math.floor(Math.random()*20); //centro do circulo
		for(y=0;y<height;y++){
			for(x=0;x<width;x++){
				dx=x-cx;
				dy=y-cy;
		...
		cx=10+Math.floor(Math.random()*20); //posicao do quadrado
		cy=10+Math.floor(Math.random()*20); //posicao do quadrado
		for(y=0;y<height;y++){
			for(x=0;x<width;x++){
