...
for(x=0;x<300000;x++){ //ajusta os bias centenas de vezes
	total_erros=0;

	for(i=0;i<30;i++){
		somatorio=bias;
        for(h=0;h<8;h++)somatorio+=weights[h]*inputs[i][h]; //para as 8 caracteristicas
		...
		if(erro!=0.0){
			total_erros++;
			for(h=0;h<8;h++)weights[h]+=0.000002*erro*inputs[i][h];
			bias+=0.000002*erro;
		}
	}

	console.log(`${300000-x} bias=${bias.toFixed(6)}`);
