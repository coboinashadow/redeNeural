...
for x in range(300000): #ajusta os bias centenas de vezes
    total_erros=0

    for i in range(30):
        somatorio=bias
        for h in range(8):somatorio=somatorio+(weights[h]*inputs[i][h]) #para as 8 caracteristicas
        ...
        erro=target[i]-degrau

        if erro!=0.0:
            total_erros=total_erros+1
            for h in range(8):weights[h]=weights[h]+(0.000004*erro*inputs[i][h])
            bias=bias+(0.000004*erro)

    print(f"{300000-x} bias={bias:.6f}")
