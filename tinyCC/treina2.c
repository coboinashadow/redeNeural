...
int main(){
	...
	for(int x=0;x<300000;x++){ //ajusta os bias centenas de vezes
		int total_erros=0;

		for(int i=0;i<30;i++){
			double somatorio=bias;
			for(int h=0;h<8;h++)somatorio+=weights[h]*dataset[i].inputs[h]; //para as 8 caracteristicas

			double degrau=somatorio>=0.0?1.0:0.0;
			double erro=dataset[i].target-degrau;

			if(erro!=0.0){
				total_erros++;
				for(int h=0;h<8;h++)weights[h]+=0.000002*erro*dataset[i].inputs[h];
				bias+=0.000002*erro;
			}
		}
