...
int main(){
	...
	for(int x=0;x<50000;x++){ //ajusta os bias centenas de vezes
		alguma sample=dataset[rand()%30]; //escolhe uma fruta qualquer

		// --- FORWARD PROPAGATION ---
		double hidden[8],output;
		for(int h=0;h<8;h++){
			hidden[h]=bias[h];
			for(int i=0;i<8;i++)hidden[h]+=sample.inputs[i]*weights[i][h];
		...
		// --- BACKPROPAGATION ---
		double outputfinal=(sample.target-output)*dsigmoid(output);
		...
		for(int i=0;i<8;i++)for(int h=0;h<8;h++)weights[i][h]+=LEARNING_RATE*hiddenfinal[h]*sample.inputs[i];
